# ALPHA IMPLEMENTATION PLAN - Catalytic Customer
...
**Changelog:**
- v1.1 (2025-10-31): Added WebSocket heartbeat/ping, Caddy env passthrough, exponential backoff reconnection, async broadcast protection, email deliverability checklist, backup container path notes
...
This document specifies the **ALPHA prototype implementation**.
Beta features are mentioned for architectural context but are **NOT implemented in Alpha**.

---

## Component Classification

### 🟢 REAL COMPONENTS (Implemented in Alpha, Used in Beta)
1. Landing page with registration form
2. Email invitation system
3. Token generation/validation (30-day expiry)
4. Session activation and timing (1-hour sessions)
5. Database (SQLite with WAL mode)
6. Admin dashboard with real-time updates
7. Google OAuth for admin login
8. Caddy reverse proxy with basic auth
9. WebSocket real-time communication
10. All timers and expiration logic

### 🟡 PLACEHOLDER COMPONENTS (Alpha Only - Replaced in Beta)
1. **Session Interface** - Test form with manual progress control (not real chat)
2. **Summary Generation** - Hardcoded placeholder text
3. **Transcript Generation** - Hardcoded placeholder text
4. **Conversation Flow** - Manual "questions_answered" simulation
5. **Session Timer** - Simple countdown display (no LLM-driven reminders)

### ❌ NOT IN SCOPE (Beta Only - Do Not Implement)
1. Real LLM conversation interface
2. AI-generated summaries from actual conversations
3. AI-generated transcripts from actual conversations
4. Open WebUI or agent application integration
5. Neo4j or other agent-specific dependencies
6. Modal/notification reminders triggered by LLM
...
**Frontend:**
- Two separate Vue.js 3 apps (monorepo pattern)
  - `frontend-public`: Landing page + success/error pages
  - `frontend-admin`: Admin dashboard with OAuth
- Vite for bundling and hot reload

**Placeholder (Alpha):**
- Separate Vue.js 3 app (`placeholder-agent`)
- Test form for simulating session progress
- Communicates with backend via API + WebSocket

**Infrastructure:**
- Caddy 2 (reverse proxy, TLS, basic auth)
- Docker Compose (single network)
- Environment-aware configuration (`.env.dev`, `.env.prod`)

### 1.2 Modular Architecture

**Main Stack:**
```
docker compose up -d  # Caddy, Backend, Frontend-Public, Frontend-Admin
```

**Placeholder Stack (Alpha):**
```
docker compose -f docker-compose.placeholder.yml up -d  # Test form agent
```

**Future Agent Stack (Beta - Reference Only):**
```
docker compose -f docker-compose.agent.yml up -d  # Real LLM agent
```

**Why Modular:** Clean separation allows dropping in real agent without touching core infrastructure.
...
## 3. Status System (Derived, Not Stored)
...
### 4.1 Registration & Invitation

**Step 1: User visits landing page**
- Route: `https://catalytic.learnstream.tech/`
- Enters: First name (required), Last name (required), Email (required)
- Clicks "Get Started"
...
### 4.2 Session Activation

**User clicks invite link:** `https://catalytic.learnstream.tech/session?token=abc123`
...
### 4.3 Placeholder Session Interface (Alpha)

**Route:** `/session/test?session_id=123`
...
- `POST /api/session/{id}/progress` - Update questions_answered
- `WS ws://backend:8000/session/{id}/stream` - Receive time updates/expiry

**WebSocket messages to placeholder:**
```json
...
**When placeholder POSTs questions_answered = 10:**
...
│                                          │
│ Summary:                                 │
│ Placeholder summary: User completed      │
│ 10 questions in alpha test.              │
│                                          │
│ Transcript:                              │
...
**Checked on every API call to session endpoints:**
```python
...
### 5.1 Access Control (Double Layer)

**Layer 1: Caddy Basic Auth**
- Three admin users: Apolo, Paul, Curtis
- Each has individual credentials (stored as bcrypt hashes in env vars)

**Layer 2: Google OAuth**
- Backend validates Google user session
- Admin must be authenticated via Google to access dashboard data
...
**Beta Caddyfile (future reference):**
```caddyfile
{$DOMAIN} {
...
### 5.2 Dashboard Display (Read-Only Table)
...
**All fields are READ-ONLY** - no inline editing.

**Token Valid Computation:**
```python
def is_token_valid(user):
    if user.token_activated:
        return False  # Already used
    return now() < user.token_expires_at
```

**Session Timer Computation:**
```python
def get_session_timer(user):
    if user.status != "Active":
        return None

    remaining = user.session_expires_at - now()
    return f"{int(remaining.total_seconds() / 60)} minutes left"
```
...
**Button:** "Add User" (top-right of dashboard)
...
│ Invitation Created                     │
├────────────────────────────────────────┤
│ Copy this URL and send to user:       │
│                                        │
│ [https://catalytic.learnstream.tech/   │
...
**No automatic email sent from admin dashboard in Alpha.**

#### Action 2: Delete User

**Button:** "Delete" (in each table row)
...
**Available for ALL statuses** (no restrictions).
...
#### Action 3: Reissue Invite

**Button:** "Reissue Invite" (in each table row)

**Enabled When:**
- Status = Incomplete (session expired, didn't finish)
- Status = Completed (wants another session)

**Disabled When:**
- Status = Invited (token not clicked yet)
- Status = Active (session still in progress)
...
**Response Modal:** Same as Add User - shows URL to copy.

### 5.4 Real-Time Updates (WebSocket)

**Admin WebSocket Endpoint:** `ws://backend:8000/admin/stream`
...
export function useAdminWebSocket() {
  ...
  const wsUrl = `${protocol}//${host}/admin/stream`;
  ...
  const connect = () => {
    ...
    ws.onmessage = (event) => {
      ...
      if (message.type === 'initial') {
        ...
        if (index !== -1) {
          users.value[index] = message.data;
        }
...
- User registered (from public form)
- User activated session (clicked token)
- User progress updated (questions_answered changed)
- User completed session
- Session expired
- User added by admin
- User deleted by admin
- Invite reissued by admin

**No Optimistic Locking:** Last write wins (acceptable for alpha with 3 admins).
...
### Frontend WebSocket Auto-Detection

**Uses `window.location` for environment-aware URLs:**
```typescript
const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
const host = window.location.host;
const wsUrl = `${protocol}//${host}/admin/stream`;
...
### 7.3 .env.example (Committed Template)
...
### 8.1 Main Stack (docker-compose.yml)
...
### 8.2 Placeholder Stack (docker-compose.placeholder.yml)
...
If scheduling backups via cron on the host machine, use Docker exec to run the backup inside the container:
...
**Note:** Ensure the `/data/backups/` directory exists and is mounted as a volume for persistence.
...
**ONLY the public registration form sends emails automatically.**

Admin dashboard actions (Add User, Reissue Invite) **DO NOT** send emails - admin manually copies URL and sends via their own email client.
...
**Prevents email spam from public form:**
```python
# Check cooldown before sending
if existing_user and (now() - existing_user.token_issued_at) < timedelta(minutes=5):
    return redirect("/error/cooldown")
```

**Cooldown error page shows:**
```
Invitation Already Sent

We recently sent an invitation to this email address.
...
**Risk Assessment:** Without SPF/DKIM configuration, production emails have 30-50% chance of being flagged as spam. Test thoroughly before launch.

**Fallback Strategy:** If email deliverability issues occur, admins can use the manual invitation flow (Add User → Copy URL → Send via personal email client).

---

## 12. Google OAuth Setup (Reference)

**Detailed setup instructions will be in separate ancillary document.**

**Quick Overview:**
1. Create Google Cloud project
2. Enable People API or Google+ API
3. Create OAuth 2.0 credentials (Web application)
4. Add authorized redirect URIs:
   - Dev: `http://localhost/api/auth/callback`
   - Prod: `https://catalytic.learnstream.tech/api/auth/callback`
5. Copy Client ID and Client Secret to `.env` files

---

## 13. WebSocket Patterns (Reference Implementation)
...
async def admin_websocket(websocket: WebSocket):
    ...
    try:
        while True:
            # Keep connection alive and respond to pongs
...
### Frontend Pattern (Vue Composable)
...
## 15. Future Enhancements (Beta - Not Implemented)

- [ ] Real LLM agent application (Open WebUI + Python agents)
- [ ] AI-generated summaries based on conversation
- [ ] AI-generated transcripts from actual Q&A
- [ ] Modal/notification reminders triggered by agent
- [ ] Admin dashboard email sending (auto-send invite)
- [ ] Resend email button for Invited users
- [ ] Neo4j integration for agent knowledge graph
